...
def save_point_cloud(pcd: o3d.geometry.PointCloud, file_path: str) -> str:
    """
    Сохраняет облако точек в .ply или .npy.
    """
    path = Path(file_path)
    if path.suffix.lower() == ".ply":
...
# ---------------- Preprocessing ----------------
def voxel_downsample(pcd: o3d.geometry.PointCloud, voxel_size: float = 0.005) -> o3d.geometry.PointCloud:
...
def preprocess_point_cloud(input_path: str, output_path: str) -> str:
    ...
    pcd = remove_plane(pcd, distance_threshold=0.01)
    pcd = crop_roi(pcd)
